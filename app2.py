    ...
    <div class="modal fade" id="previewModal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="tab-menu">
                    <button class="tab-btn active" onclick="switchTab('preview')"><i class="bi bi-eye"></i> Preview</button>
                    <button class="tab-btn" onclick="switchTab('download')"><i class="bi bi-download"></i> Download</button>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <div id="downloadTab" class="tab-content">
                        <div style="text-align: center;">
                            <p style="margin-bottom: 20px;">Ready to download <strong id="downloadFileName"></strong></p>
    ...
    <script>
                        ...
                        container.innerHTML = '<ul class="file-list">' + files.map(f => `
                            <li class="file-item">
                                ...
                                <div class="file-actions">
                                    <button class="btn-preview" onclick="previewFile('${f}')">
                                        <i class="bi bi-eye"></i> Preview
                                    </button>
                                    <button class="btn-download" onclick="downloadFileAction('${f}')">
        ...
        function previewFile(name) {
            ...
            fetch(`/preview?name=${encodeURIComponent(name)}`)
                ...
                .catch(e => {
                    document.getElementById('previewContent').innerHTML = '<p style="color: #999; text-align: center; padding: 40px;">Preview not available for this file type</p>';
                    previewModal.show();
                });
        }
        ...
        function downloadFileAction(name) {
            currentPreviewFile = name;
            document.getElementById('previewTitle').textContent = 'Download: ' + name;
            document.getElementById('downloadFileName').textContent = name;
            switchTab('download');
            previewModal.show();
        }
        ...
        function downloadFile() {
            ...
            window.location.href = `/download?name=${encodeURIComponent(currentPreviewFile)}`;
        }
...
def preview_file():
    try:
        ...
        blob_service = BlobServiceClient.from_connection_string(STORAGE_CONNECTION_STRING)
        blob_client = blob_service.get_blob_client(CONTAINER_NAME, name)
        content = blob_client.download_blob().readall()
        
        if ext in ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp']:
            b64 = base64.b64encode(content).decode('utf-8')
            preview_html = f'<img src="data:image/{ext};base64,{b64}" class="preview-image">'
        elif ext == 'txt':
            text = content.decode('utf-8', errors='ignore')
            preview_html = f'<div class="preview-text">{text}</div>'
        elif ext == 'pdf':
            b64 = base64.b64encode(content).decode('utf-8')
